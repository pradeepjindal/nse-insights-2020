...
public class DmMerger {
    ...
    public void merge(List<PraBean> praBeans, LocalDate forDate) {
        ...
        fromFile = praFileUtils.getLatestFileNameFor(ApCo.DM_FILES_PATH, ApCo.PRA_DM_FILE_PREFIX, ApCo.REPORTS_FILE_EXT,1, forDate);
        Map<String, DmBean> mtLatestBeanMap = csvReader.read(fromFile);
        //fromFile = fileUtils.getLatestFileNameForMat(2);
        fromFile = praFileUtils.getLatestFileNameFor(ApCo.DM_FILES_PATH, ApCo.PRA_DM_FILE_PREFIX, ApCo.REPORTS_FILE_EXT,2, forDate);
