...
public class CmMerger {
    ...
    public void merge(List<PraBean> praBeans, LocalDate forDate) {
        ...
        fromFile = praFileUtils.getLatestFileNameFor(NseCo.CM_FILES_PATH, ApCo.PRA_CM_FILE_PREFIX, ApCo.REPORTS_FILE_EXT,1, forDate);
        Map<String, CmBean> latestBeanMap = csvReader.read(fromFile);
        fromFile = null;
        //fromFile = fileUtils.getLatestFileNameForCm(2);
        fromFile = praFileUtils.getLatestFileNameFor(NseCo.CM_FILES_PATH, ApCo.PRA_CM_FILE_PREFIX, ApCo.REPORTS_FILE_EXT,2, forDate);
